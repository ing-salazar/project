...
# function to get the total amount of posts in the database 
def get_post_count():
    connection = sqlite3.connect('database.db')
    post_count = len(connection.execute('SELECT * FROM posts').fetchall())
    return post_count

# Function to get the title of a post using its ID
...
def create():
    if request.method == 'POST':
        ...
        if not title:
            ...
            app.logger.info('A new ' + title + ' article was created')
